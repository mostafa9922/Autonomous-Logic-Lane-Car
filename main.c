...
int main(void)
{
  ...
  TCS3200_Init();		      // Initialize the color sensor
  IR_Sensor_Init();           // Initialize IR sensors

    // Start PWM Channels for motor speed control
        HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1);
        HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_2);
         uint8_t rightIR1 = IR_Sensor1_Read();
         uint8_t rightIR2 = IR_Sensor2_Read();
         uint8_t leftIR1 = IR_Sensor3_Read();
         uint8_t leftIR2 = IR_Sensor4_Read();
